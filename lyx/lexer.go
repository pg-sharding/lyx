...
//line lyx/lexer.rl:299
		lex.te = (lex.p) + 1
		{
			lval.str = string(lex.data[lex.ts+1 : lex.te-1])
			tok = IDENT
			{
				(lex.p)++
				lex.cs = 9
...
//line lyx/lexer.rl:102
		...
		(lex.p)--
		{
			lval.int, _ = strconv.Atoi(string(lex.data[lex.ts:lex.te]))
			tok = ICONST
			{
				(lex.p)++
				lex.cs = 9
